...
# Cada objeto dessa classe guarda seus proprios dados (nome,categoria, ativo).

class Restaurante:

    # Construtor: roda automaticamente quando um novo Restaurante é criado
    #Recebe nome e categoria, e já define ativo como false por padão
    ...
    #Não recebe parametros alem de self porque so mexe nos proprios dados.
    ...
    #formação toda vez que for exibir um restaurante na tela.
    def __str__(self):
        status = "Ativo" if self.ativo else "Inativo"
        return f"-{self.nome.ljust(20)} | {self.categoria.ljust(20)} | {status}"

# Classe principal do sistema. Guarda a lista de restaurante e concentra
...
class SaborExpress:
    ...
    def __init__(self):
        self.restaurantes = [
            ...
            Restaurante("Pizza Suprema", "Pizzaria"),
            Restaurante("Cantina", "Italiano"),

        ]

        # Criar um novo objeto Restaurante e adiciona na lista interna.
    ...
    # A formatação da exibição é responsabilidadeda classe Menu.
...
def listar_restaurantes(self):
    ...
    # print(restaurante) aqui já usa o _str_ definido na classe Restaurante,
...
#Le a opção digitada e decide qual método chamr 
...
# diretamente (não quandofor importado por outro arquivo).
if _name_ == "_main_":
    menu = Menu()
    menu.main()
